...
const getImageUrl = (userId: string, propertyId: string): string => {
  ...
  const { sdata, serror } =  supabase.storage.from('landlords')
  .createSignedUrl(`landlord/${userId}/property_img/${propertyId}/`, 1700)
  if (sdata) {
    console.log("SDATA     >>", sdata.signedUrl)
  }else if (serror) {
    throw new Error("SERROR   >>",serror.message)
  }

  const { data, error } = supabase.storage
    .from('landlords')
    .getPublicUrl(`landlord/${userId}/property_img/${propertyId}/`)
